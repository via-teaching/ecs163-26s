...
function renderBarChart(data) {
    ...
    // yearly salary average - organize data by year
    const yearlyAvg=d3.groups(data, d=>d.work_year)
                       .map(([year, v])=>({year, avg: d3.mean(v, d=>d.salary_in_usd)}))
                       .sort((a,b)=>a.year-b.year);

    // the following function calls set a margin for the dashboard
    ...
    const y=d3.scaleLinear().range([height, 0]).domain([0, d3.max(yearlyAvg, d=>d.avg)]).nice(); // maps numeric salary to pixel height
...
function renderPieChart(data, colors) {
    ...
    const svg=d3.select("#pie-svg-container").append('svg').attr('width', width).attr('height', height); // sets a dashboard margin
...
function renderArcDiagram(data, colors) {
    ...
    d3.select('#arc-svg-container').selectAll('*').remove(); // set margin for the dashboard
