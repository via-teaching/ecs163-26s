...
function drawSankey(svg, rows, layout) {
    ...
    const margin = { top: 48, right: 24 + legendWidth, bottom: 36, left: 24 };
    ...
    function updateSankeyHighlight(hoveredNode) {
        linkSelection
            .attr("stroke-opacity", function (d) {
                return isSankeyLinkHighlighted(d, hoveredNode) ? 0.78 : 0.07;
            })
            .attr("stroke-width", function (d) {
                const base = Math.max(1, d.width);
                return isSankeyLinkHighlighted(d, hoveredNode) ? base * 1.08 : base * 0.55;
            });

        node.select("rect")
            .attr("opacity", function (d) {
                return isSankeyNodeHighlighted(d, hoveredNode, links) ? 1 : 0.22;
            });

        node.select("text")
            .attr("opacity", function (d) {
                return isSankeyNodeHighlighted(d, hoveredNode, links) ? 1 : 0.3;
            });
    }
