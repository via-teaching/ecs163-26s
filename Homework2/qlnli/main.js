...
//step 2: grab the svg canvas from index.html
...
d3.csv("data/student-mat.csv").then(function(data){
    ...
    //step 4: draw Scatter Plot(context view)
    ...
    //step 5: draw Bar Chart(focus view 1)
    ...
    //step 6: draw Parallel Coordinates(focus view 2, advanced)
    ...
    //step 7: add brushing and linking
    //add a brush to the scatter plot to allow users to select a region
    const brush = d3.brush()
        .extent([[0, 0], [scatterWidth - 100, scatterHeight - 100]])
        .on("end", updateCharts);
    //append the brush to the scatter plot group
    scatterGroup.append("g")
        .attr("class", "brush")
        .call(brush);
    /*function to update the bar chart and parallel coordinates plot
    based on the selected region in the scatter plot*/
    function updateCharts(){
        //get the selected region from the brush event
        const extent = d3.event.selection;
        //if no region is selected, reset all charts to show the full dataset
        if(!extent){
            scatterGroup.selectAll("circle").style("fill", "steelblue");
            updateBarChart(data);
            //reset parallel coordinates plot to show all lines
            pcGroup.selectAll("path").style("stroke", "steelblue").style("display", null);
            return;
        }
        //filter the data to include only points that fall within the selected region
        const selectedData = data.filter(d => {
            const cx = xScaleScatter(d.absences);
            const cy = yScaleScatter(d.G3);
            //check if the point's coordinates fall within the brush extent
            return extent[0][0] <= cx && cx <= extent[1][0] && 
                   extent[0][1] <= cy && cy <= extent[1][1];
        });
        //update the scatter plot to highlight selected points and dim unselected points
        scatterGroup.selectAll("circle")
            .style("fill", function(d){
                const cx = xScaleScatter(d.absences);
                const cy = yScaleScatter(d.G3);
                //check if the point is within the brush extent to determine its color
                const isSelected = extent[0][0] <= cx && cx <= extent[1][0] && 
                                   extent[0][1] <= cy && cy <= extent[1][1];
                return isSelected ? "red" : "lightgray";
            });
        //update the bar chart to show averages based on the selected data
        updateBarChart(selectedData);
        /*update the parallel coordinates plot to highlight lines 
        corresponding to the selected data*/
        pcGroup.selectAll("path")
            .style("display", function(d){
                //check if the data point corresponding to this line is in the selected data
