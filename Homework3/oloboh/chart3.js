...
//sankey w/ selection interaction & transiiton
...
//track which node is currently selected (null = none)
...
d3.csv("data/survey_605.csv").then(function(rawData) {
    ...
    //give each node a unique name to stop overlaps
    ...
    node_g.append("rect")
        ...
        .on("click", function(d) { 
            if (selected_node === d) { //if selected already...
                selected_node = null;
                reset_highlight();
            } else {
                selected_node = d;
    ...
    //custom node highlighting func
    function highlight_node(node) {
        link_paths
            .transition() //animation
            .duration(1000) //transition/animation length
            .attr("stroke-opacity", function(d) { //dont make immediate touching nodes transparent
                const is_connected = d.source === node || d.target === node;
                return is_connected ? 0.8 : 0.05;
            });
    }

    //custom func 2 reset highlight
