...
//histogram w/ paning & zooming interaction
...
// help stops clipping when panning
svg.append("defs").append("clipPath")
    .attr("id", "clip-c1-outer")
    .append("rect")
    .attr("x", -margin_c1.left + 52)
    .attr("y", -margin_c1.top)
    .attr("width", width_c1 + margin_c1.left + margin_c1.right)
    .attr("height", height_c1 + margin_c1.top + margin_c1.bottom + 80);
g1.attr("clip-path", "url(#clip-c1-outer)");
...
d3.csv("data/survey_605.csv").then(function(rawData) {
    ...
    //x y drawing with zooming
    const x_axis_g = g1.append("g")
        .attr("class", "x-axis")
        .attr("transform", `translate(0, ${height_c1})`);
    x_axis_g.call(d3.axisBottom(x0))
        .selectAll("text")
        .attr("dy", "1em")
    ...
    const bars_g = g1.append("g")
        .attr("class", "bars-group")
        .attr("clip-path", "url(#clip-c1)");

    //draw age group sub-groups inside the clipped bars group
    const age_groups = bars_g.selectAll(".age-group")
        .data(order_age)
        .enter()
        .append("g")
    ...
    //zoom handling
    const zoom_c1 = d3.zoom()
        .scaleExtent([1, 6])
        .translateExtent([[0, 0], [width_c1, height_c1]]) //panning bounds
        .extent([[0, 0], [width_c1, height_c1]])
        .on("zoom", zoomed_c1);

    //zoom vision bounds & miuse reading
    g1.append("rect")
    ...
    function zoomed_c1() {
        ...
        //zoom scale using original range
        const zoomed_x0 = x0.copy().range(
            x0.range().map(d => transform.applyX(d))
        );

        //move aroudn age groups during zoom
        bars_g.selectAll(".age-group")
            .attr("transform", d => `translate(${zoomed_x0(d)}, 0)`);

        //bar scaling during zoom
        const zoomed_x1 = x1.copy().range( [0, zoomed_x0.bandwidth()] );
        bars_g.selectAll(".age-group").each(function() {
            d3.select(this).selectAll("rect")
                .attr("x", d => zoomed_x1(d.freq))
                .attr("width", zoomed_x1.bandwidth());
        });

        //axis redraw
        x_axis_g.call( d3.axisBottom(zoomed_x0) ).selectAll("text")
            .attr("dy", "1em")
            .style("font-size", "12px");
    }
