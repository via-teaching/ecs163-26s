...
// The interactions I used for HW3:
// 1. I used Brushing on a bar chart to select genres and scatter plot updates with animation transitions
// 2. Pan and zoom on the parallel coordinates to scroll/drag to explore
// 3. Animated transition - the scatter dots fade in and out when the genre filter changes
...
// HW3: global set to track which genres are selected by the brush
// empty set means all genres are shown
let selectedGenres = new Set();

// HW3: global reference to the scatter update function
// gets set after drawScatter runs so the bar chart brush can call it
let updateScatter = null;
// no filter is applied
// all genres are shown
// scatter plot highlights only those genres
...
function drawBarChart(data) {
    ...
    // HW3: hint label so user knows they can drag to filter
    g.append("text")
        .attr("x", W)
        .attr("y", -4)
        .attr("text-anchor", "end")
        ...
        .text("← drag to filter genres");

    // HW3: brushing interaction
    // d3.brushX lets the user drag a horizontal selection over the bars
    // when the brush moves, we find which genres fall under it
    // and update selectedGenres, then call updateScatter()
    const brush = d3.brushX()
        .extent([[0, 0], [W, H]])
        .on("brush end", function(event) {

            if (!event.selection) {
                // no selection means show all genres
                selectedGenres.clear();
            } else {
                const [x0, x1] = event.selection;
                selectedGenres.clear();
                genreData.forEach(function(d) {
                    const barCenter = x(d.genre) + x.bandwidth() / 2;
            ...
            // dim unselected bars to give visual feedback
            bars.attr("opacity", function(d) {
                if (selectedGenres.size === 0) return 1;
                return selectedGenres.has(d.genre) ? 1 : 0.25;
            });

            // call scatter update with animated transition
            if (updateScatter) updateScatter();
        });

    // append brush layer on top of bars so it captures mouse events
    g.append("g")
        .attr("class", "brush")
        .call(brush);
}
...
function drawScatter(data) {
    ...
    // HW3: animated transition function
    // called by the bar chart brush whenever genre selection changes
    // dots for selected genres fade in, others fade out
    updateScatter = function() {
        dots.transition()
            ...
            .attr("opacity", function(d) {
                ...
                return selectedGenres.has(d["Fav genre"]) ? 0.85 : 0.04;
            })
            .attr("r", function(d) {
                if (selectedGenres.size === 0) return 3.5;
                return selectedGenres.has(d["Fav genre"]) ? 4.5 : 2.5;
            });
    };
...
function drawParallel(data) {
    ...
    // HW3: zoom behavior - scroll to zoom, drag to pan
    // scale extent limits how far in/out user can zoom
    const zoom = d3.zoom()
        .scaleExtent([0.5, 5])
    ...
    // apply zoom to the svg so the whole chart area is zoomable
    svg.call(zoom);

    const g = svg.append("g")
        .attr("transform", `translate(${margin.left},${margin.top})`);

    // HW3: separate inner group that gets transformed by zoom/pan
    // outer g keeps the margin fixed, inner g moves with zoom
    ...
    // Draw one line per respondent into the zoomable inner group
    innerG.selectAll("path.par-line")
        .data(parData)
        .enter()
        .append("path")
            .attr("class", "par-line")
    ...
    // Draw each vertical axis into the zoomable inner group
    dimensions.forEach(function(dim) {
        const axisG = innerG.append("g")
    ...
    // Genre color legend (outside zoom group so it stays fixed)
