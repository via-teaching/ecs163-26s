/* 
    AI disclosure: ChatGPT was used to identify the strengths of different transitions and help study and debug D3 code.
    ChatGPT also helped with debugging interactivity and animation code. I reviewed, adapted, and integrated the code myself.
...
let clearLineBrush = null;
let ignoreBrushEnd = false;

function clearBrushWithoutUpdatingState() {
    if (!clearLineBrush) {
        return;
    }
    ...
    ignoreBrushEnd = true;
    clearLineBrush();
    ignoreBrushEnd = false;
}

function focusDescription() {
    if (interactionState.selectedSymptom !== null) {
        return `Current focus: ${burdenLabels[interactionState.selectedSymptom]}`;
    }

    if (interactionState.brushedYears) {
        return `Current focus: ${interactionState.brushedYears.join(", ")}`;
    }

    return "Current focus: All students";
}

function updateFocusLabel() {
    d3.select("#focus-label").text(focusDescription());
}

function clearFocus() {
    interactionState.selectedSymptom = null;
    interactionState.brushedYears = null;

    clearBrushWithoutUpdatingState();
    updateHighlights();
}

function selectSymptom(score) {
    clearBrushWithoutUpdatingState();

    interactionState.selectedSymptom = interactionState.selectedSymptom === score ? null : score;
    interactionState.brushedYears = null;

    updateHighlights();
}

function brushYears(years) {
    interactionState.brushedYears = years && years.length ? years : null;
    interactionState.selectedSymptom = null;
...
function isActiveYear(year) {
    if (interactionState.brushedYears) {
        return interactionState.brushedYears.indexOf(year) >= 0;
    }
...
function updateHighlights() {
    const selectedSymptom = interactionState.selectedSymptom;
    const brushedYears = interactionState.brushedYears;

    updateFocusLabel();

    // Animates focus change in the alluvial nodes
    svg.selectAll(".flow-node")
        .transition()
        .duration(transitionMs)
        .attr("opacity", function(d) {
            if (brushedYears && d.layerKey === "year") {
                return brushedYears.indexOf(d.rawValue) >= 0 ? 1 : 0.25;
            }

            if (selectedSymptom !== null && d.layerKey === "symptoms") {
                return d.rawValue === selectedSymptom ? 1 : 0.25;
            }

            return 1;
        });

    // Animates focus change in the alluvial links
    svg.selectAll(".flow-link")
        .transition()
        .duration(transitionMs)
        .attr("stroke-opacity", function(d) {
            if (brushedYears) {
                if (d.stage === 0) return brushedYears.indexOf(d.source.rawValue) >= 0 ? 0.7 : 0.1;
                return 0.2;
            }

            if (selectedSymptom !== null) {
                return d.symptomScore === selectedSymptom ? 0.7 : 0.1;
            }

            return 0.45;
        });

    // Animates focus change in the donut overview
    svg.selectAll(".donut-slice")
        .transition()
        .duration(transitionMs)
        .attr("opacity", function(d) {
            if (selectedSymptom !== null) {
                return d.data.score === selectedSymptom ? 1 : 0.25;
            }

            return 1;
        });

    // Animates focus change in the line-chart points
    svg.selectAll(".indicator-point")
        .transition()
        .duration(transitionMs)
        .attr("r", function(d) {
            return brushedYears && isActiveYear(d.year) ? 6 : 4.2;
        })
        .attr("opacity", function(d) {
            return brushedYears && !isActiveYear(d.year) ? 0.25 : 1;
        });

    // Slightly fades lines when a year subset is active
    svg.selectAll(".indicator-line")
        .transition()
        .duration(transitionMs)
        .attr("opacity", brushedYears ? 0.75 : 1);
}
...
function drawDashboard(data) {
    ...
    // Gives the user a simple guide to the dashboard interactions
    svg.append("text")
        .attr("x", outerMargin)
        .attr("y", 64)
        .attr("font-size", 11)
        .attr("fill", mutedText)
        .text("Click a symptom group to focus. Brush across years in the line chart to compare a subset.");

    // Shows the current selected or brushed focus
    svg.append("text")
        .attr("id", "focus-label")
        .attr("x", width - outerMargin - 72)
        .attr("y", 64)
    ...
    // Clears selection and brushing so the full context is visible again
    svg.append("text")
        .attr("x", width - outerMargin)
        .attr("y", 64)
        .attr("text-anchor", "end")
        .attr("font-size", 11)
        .attr("font-weight", 700)
...
function drawAlluvial(data) {
    ...
    const nodes = g.selectAll(".flow-node")
        ...
        .style("cursor", d => d.layerKey === "symptoms" ? "pointer" : "default");
    
    nodes.filter(d => d.layerKey === "symptoms")
        .on("click", function(d) {
            d3.event.stopPropagation();
            selectSymptom(d.rawValue);
        });
...
function drawLineChart(data) {
    ...
    const brush = d3.brushX()
        ...
        .on("end", function() {
            ...
            const selectedYears = yearOrder.filter(year => {
                const position = x(year);
                return position >= selection[0] && position <= selection[1];
            });
        
            brushYears(selectedYears);
        });
