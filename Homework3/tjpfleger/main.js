//ideas:
//selecting: hovering over pokemon block gives info for that pokemon (star chart)
//brushing: hovering over type bar changes chord diagram to focus that type
//zoom: zoom into something (if needed, bar chart)
//animation: star plot values slide into place
...
d3.csv("data/pokemon_alopez247.csv").then(rawData =>{
    ...
    for(const entry of rawData){
        ...
        //record individualCountData
        individualCountData.push({
            info:entry, type:entry.Type_1, position:countDatatemp.get(entry.Type_1)
        });
        if(entry.Type_2 != ""){
            individualCountData.push({
                info:entry, type:entry.Type_2, position:countDatatemp.get(entry.Type_2)
            });  
        }
    }
    ...
    console.log("individualCountData", individualCountData);
    ...
    //const zoomableSvg = d3.create("svg")
    ...
    const g1 = svg.append("g")
                ...
                .attr("transform", `translate(${scatterMargin.left}, ${scatterMargin.top + height/2})`);

    // X label- Number of Pokemon
    g1.append("text")
    .attr("x", scatterWidth / 2)
    .attr("y", height/2-scatterMargin.bottom)
    .attr("font-size", "20px")
    ...
    //Y axis- band scale with correct bounds
    const y1 = d3.scaleBand()
    .domain(typesArray)
    .range([scatterMargin.top, height/2-(scatterMargin.top + scatterMargin.bottom)]);
    ...
    // X axis- linear scale with correct bounds
    const x1 = d3.scaleLinear()
    .domain([0, d3.max(countData, d => d.count)])
    .range([0, width-(scatterMargin.left+scatterMargin.right)]);
    const xAxisCall = d3.axisBottom(x1)
                        .ticks(13);
    g1.append("g")
    .attr("transform", `translate(0, ${height/2-(scatterMargin.top+scatterMargin.bottom)})`)
    .call(xAxisCall);

    //plot 1 bars- create a bar for each pokemon in individualCountData, set height to number of pokemon
    const singlePokemonWidth = (width-(scatterMargin.left+scatterMargin.right)) / 125;
    const marks = g1.selectAll("rect")
    .data(individualCountData)
    .join("rect")
    .attr("y", d => y1(d.type))
    .attr("x", d => x1(d.position))
    .attr("height", height/70)
    .attr("width", d => singlePokemonWidth)
    .attr("fill", (d, i) => {
        //return Math.floor(i / 11) == 0 ? color(d.type) : color(d.type);
        return color(d.type);
    })
    .on("mouseover", function(pokemon, i, rects){
        //let pokemon = individualCountData[i];
        //console.log(event);
        //display name
        nameLabel.transition()
        .attr("x", x1(pokemon.position))
        .attr("y", y1(pokemon.type)-5)
        .text(pokemon.info.Name);
        //create plot 2
        //radial line
        //graphs onto each chart the stat data as the radius and the fixes axis angles as the angles
        chart2title.text(pokemon.info.Name);
        let pokemonStats = [];
        pokemonStats.push(pokemon.info.HP);
        pokemonStats.push(pokemon.info.Attack);
        pokemonStats.push(pokemon.info.Defense);
        pokemonStats.push(pokemon.info.Sp_Atk);
        pokemonStats.push(pokemon.info.Sp_Def);
        pokemonStats.push(pokemon.info.Speed);
        let lineData = [];
        for(let i = 0; i < 6; i++){
            lineData.push({
                angle:angle[i], radius:pokemonStats[i]
            });
        }
        ...
        .transition()
        .attr("d", line)
        .attr("fill", color(pokemon.type))
        .attr("stroke", color(pokemon.type))
        .attr("stroke-width", 3)
        .attr("opacity", 0.8)
        .attr("transform", `translate(${chartX}, ${chartY})`);
    });
    const nameLabel = g1.append("text")
    .attr("font-size", 26)
    .attr("text-anchor", "middle");
    /*const marks = g1.selectAll("rect")
    .data(countData)
    ...
    g1.append("text")
        ...
        .attr("y", scatterTop+scatterMargin.top/2)
        .attr("font-size", 20)
        .attr("text-anchor", "middle")
        .text("Number of Pokemon by Type");

    //box to drag brush region
    g1.append("rect")
...
let boxY = 30;
function dragged(d) {
    //const boxSelectedType = y1.invert(boxY);
    //[scatterMargin.top, height/2-(scatterMargin.top + scatterMargin.bottom)]
    //console.log(boxSelectedType);
    boxY = d3.event.y;
    d3.select(this).attr("y", boxY);
    let boxSelectedType = typesArray[0];
    for(let i = 0; i < typesArray.length; i++){
        if(y1(typesArray[i]) > boxY - (height/100)){
            boxSelectedType = typesArray[i];
            break;
        }
    }
    //modify plot 3
    ...
    .data(connections)
    .join("path")
    .attr("fill", d => color(names[(names[d.source.index] != boxSelectedType) ? d.source.index : d.target.index]))
    .attr("d", ribbon)
    .attr("transform", `translate(${width/9}, ${height*0.23})`)
    .style("mix-blend-mode", "multiply")
    .attr("opacity", d => (names[d.source.index] == boxSelectedType || names[d.target.index] == boxSelectedType) ? 1.0 : 0.1);
...
const outerRadius = height/5.3;
    ...
    let chartNum = 0;
        ...
        //chart titles
        const chart2title = g2.append("text")
        .attr("x", chartX)
        .attr("y", chartY-radius-20)
        .attr("font-size", 20)
        .attr("text-anchor", "middle")
        .text("Hover Over Bar Graph");
    ...
    .attr("y", 270)
    .attr("font-size", 20)
    .text("Drag grey box to brush type")
    svg.append("text")
    .attr("x", width*11/16)
    .attr("y", 290)
    .attr("font-size", 20)
    .text("Hover over bars to select pokemon")

    //brushing
    /*
    //d3.select("body").on("keydown", function(){
    svg.call(d3.brush().extent([[0,height/2],[width,height]])
    .on("start brush end", (d) => {
        //console.log("hi")
    }))
    //});
    */
