...
// New variable that will keep track of the type for the interactive features in my visualizations
...
// d3 function select uses the exisiting element 
...
d3.csv("data/pokemon_alopez247.csv").then(function(dataset) {
    ...
    // Convert the columns we need into numerical values bc loaded as strings by default
...
function draw_heatmap(data, topTypes) {
    ...
    g.selectAll("rect")
        ...
        // Clicking on a type will filter/highlight the dashboard
        .on("click", function(d) {
            if (selected_type === d.type) {
                selected_type = null;
            } else {
...
function draw_scatterplot(data, color) {
    ...
    // d3 functions creates tooltop that follows the mouse 
    ...
    // d3 function to create an interactive rectangle tool for selecting certain features/points
    // User can drag this rectangle across the scatterplot to look at different regions or select Pokemon
    const brush = d3.brush()
        // Define what areas can be brushed or selected
        .extent([[0, 0], [chart_width, chart_height]])
        .on("end", brushed);

    // Add this interaction to the scatterplot
    g.append("g")
        .attr("class", "brush")
        .call(brush);

    // Draw circles after adding the layer for the brush, so that we can still hover over data points and get information
    g.selectAll("circle")
        .data(data)
        .enter()
        .append("circle")
    ...
    // Function to handle the brushing interaction of the scatterplot and make points that are not selected fade
    function brushed() {
        // d3 function selection stores the coordinates of the rectangle selected on the screen 
        const selection = d3.event.selection;

        if (!selection) {
            svg.selectAll(".pokemon-circle")
                .transition()
                .duration(600)
                .attr("opacity", 0.65);
            return;
        ...
        // Store the coordinates of the brush 
        const x0 = selection[0][0];
        const y0 = selection[0][1];
        const x1 = selection[1][0];
        const y1 = selection[1][1];

        // Select the Pokemon circles and make the others fade based on whteher they are inside or outside of the brushed rectangle
        svg.selectAll(".pokemon-circle")
            .transition()
            .duration(600)
...
function draw_legend(topTypes, color) {
    ...
    // Add a section to explain features in the static visualizations, to provide context
    legend.append("text")
        .attr("x", 0)
        .attr("y", 42)
        .attr("font-size", "18px")
        .attr("font-weight", "bold")
        .text("About the Visualizations:");

    legend.append("text")
        .attr("x", 0)
        .attr("y", 62)
        .attr("font-size", "18px")
        .text("Scatterplot: size of the circles represents HP");
    ...
    // Add to the bottom of the view to give a quick summary of the interations they can use 
    // Clearly explain how to use the interactive features and what they do
    legend.append("text")
        .attr("x", 800)
        .attr("y", 42)
        .attr("font-size", "18px")
    ...
    legend.append("text")
        ...
        .attr("y", 62)
        .attr("font-size", "18px")
        .text("1. Hover over scatterplot points to view additional information about the Pokemon");

    legend.append("text")
        .attr("x", 800)
        .attr("y", 82)
        .attr("font-size", "18px")
        .text("2. Drag over the scatterplot to brush/select a subset of Pokemon");

    legend.append("text")
        .attr("x", 800)
        .attr("y", 102)
        .attr("font-size", "18px")
        .text("3. Select heatmap cells to filter and highlight Pokemon type across displays and visualizations");
}

// Function to update the visualizations for the interactions when a type is selected or not selected 
function update_selection() {
    // Create some animated transition for the scatterplot circles
    svg.selectAll(".pokemon-circle")
        .transition()
        .duration(600)
        .attr("opacity", function(d) {
            if (selected_type === null || d.Type_1 === selected_type) {
                return 0.75;
            } else {
                return 0.08;
            }
    ...
    svg.selectAll(".sankey-link")
        ...
        .duration(600)
        .attr("opacity", function(d) {
            if (selected_type === null || d.type === selected_type) {
                return 0.3;
            } else {
                return 0.05;
            }
...
function draw_sankey_curve(g, x1, y1, x2, y2, thickness, color, type) {
    const path = d3.path();
...
HW 3:
Similarly, I used AI tools to support my initial understanding of what the various types of interactions that 
were listed on the assignment instructions actually do. I was not familiar with some of the interaction types, so it 
was useful in figuring out which ones I wanted to implement for this assignment with my specific visualizations.
*/