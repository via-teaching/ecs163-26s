...
function updateLabels() {
  ...
  d3.select("#map-subtitle").text(mapSubtitle());
  d3.select("#profile-subtitle").text("Top attack profiles for current filters");
}
...
function drawTrend() {
  ...
  const brush = d3.brushX()
    ...
    .on("end", function() {
      ...
      if (!selection) {
        state.startYear = YEAR_MIN;
        state.endYear = YEAR_MAX;
      } else {
        const years = selection.map(x.invert).map(Math.round);
        state.startYear = clamp(Math.min(years[0], years[1]), YEAR_MIN, YEAR_MAX);
        state.endYear = clamp(Math.max(years[0], years[1]), YEAR_MIN, YEAR_MAX);
      }
      updateAll();
    });
  ...
  g.append("g")
    .attr("class", "brush")
    .call(brush)
    .call(brush.move, [x(state.startYear), x(state.endYear)]);
...
function drawMap() {
  ...
  const g = svg.append("g").attr("transform", `translate(${m.left},${m.top})`);
  const zoomLayer = g.append("g").attr("class", "map-zoom-layer");
  ...
  zoomLayer.selectAll(".country-point")
    ...
    .attr("stroke", d => isSelectedCountry(d) ? "#111827" : "#243447")
    .attr("stroke-width", d => isSelectedCountry(d) ? 2.2 : 0.45)
    .attr("fill-opacity", 0)
    .on("mouseover", function(d) {
    ...
    .on("mouseout", function() {
      ...
      d3.select(this)
        .attr("stroke-width", isSelectedCountry(d) ? 2.2 : 0.45)
        .attr("fill-opacity", 0.72);
      hideTooltip();
    })
    .on("click", function(d) {
      d3.event.stopPropagation();
      state.selectedCountry = isSelectedCountry(d) ? null : d.country;
      updateAll();
    })
    .transition()
  ...
  svg.call(zoom).on("click", function() {
    if (!state.selectedCountry) return;
    state.selectedCountry = null;
    updateAll();
  });

  const selectedCountry = data.find(d => isSelectedCountry(d));
  ...
  note.append("text")
    ...
    .attr("x", 10)
    .attr("y", 18)
    .text(selectedCountry ? truncate(selectedCountry.country, 24) : `Top ${metricLabel(state.metric).toLowerCase()}`);
  note.selectAll(".top-country")
    .data(selectedCountry ? selectedCountrySummary(selectedCountry) : topCountries.map(d => `${truncate(d.country, 20)}: ${numberFormat(d[state.metric])}`))
    .enter()
    .append("text")
...
function mapSubtitle() {
  const selected = state.selectedCountry ? `, selected ${state.selectedCountry}` : "";
  return `${metricLabel(state.metric)} by country${selected}`;
}
...
function isSelectedCountry(d) {
  return state.selectedCountry && d.country === state.selectedCountry;
}

function selectedCountrySummary(d) {
  return [
    `Region: ${truncate(d.region, 24)}`,
    `Incidents: ${numberFormat(d.incidents)}`,
    `Fatalities: ${numberFormat(d.killed)}`,
    `Wounded: ${numberFormat(d.wounded)}`,
    `Fatalities / incident: ${d3.format(".2f")(d.fatalityRate)}`
  ];
}

function validateSelectedCountry() {
  if (!state.selectedCountry) return;
  const visible = countryYearData.some(d => inYearRange(d) && inRegion(d) && d.country === state.selectedCountry);
  if (!visible) state.selectedCountry = null;
}
