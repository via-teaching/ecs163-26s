...
// HW3: 
// Set of student_ids currently inside the brush rectangle (null = no brush active)
let brushed_ids = null;
// student_id that has been click-pinned in the parallel coordinates view (null = unpinned)
let pinned_student_id = null;
...
const brush_badge = d3.select("#brushBadge");
const pin_label = d3.select("#pinLabel");

// simplified from HW2. comma seperated csv files.
// d3.csv() can handle that. no need for customer loader.
Promise.all([
  d3.csv("data/student-mat.csv",  d => cleanRow(d, "math")),
  d3.csv("data/student-por.csv",  d => cleanRow(d, "portuguese"))
]).then(([math_rows, portuguese_rows]) => {
  all_rows = [...math_rows, ...portuguese_rows];

  // d3 listens for dropdown changes and triggers the filtering animation then redraws
  subject_filter.on("change", () => {
    // clear brush and pin when dataset changes so stale selections don't carry over 
    brushed_ids       = null;
    pinned_student_id = null;
    updateBrushBadge();
    updatePinLabel();
    renderDashboardAnimated();
  });
  ...
  d3.selectAll(".context-card")
    .on("click", function () {
      ...
      // clear interaction state when layout changes so old highlights don't linger
  ...
  // escape key clears both brush and pin from anywhere on the page so users don't have to unselect the specific datapoint to clear
  ...
  // clicking the brush badge clears the brush selection
  brush_badge.on("click", () => {
    brushed_ids = null;
    updateBrushBadge();
    applyGlobalHighlight();
    // re-render scatter to remove the visible brush rectangle
    renderDashboard();
  });

  // clicking the pin label unpins the student
  pin_label.on("click", () => {
    pinned_student_id = null;
    updatePinLabel();
    applyGlobalHighlight();
...
function cleanRow(row, subject) {
  ...
  // stable id assigned here so the same student can be identified across all three charts
...
function renderDashboard() {
  ...
  // reapply hover/brush/pin state after redraw so linked views stay in sync
  applyGlobalHighlight();
}

//  animated redraw used when the dataset dropdown changes 
// implements a Filtering transition: existing marks fade out, new marks fade in
function renderDashboardAnimated() {
  const FADE_OUT = 550; // ms: how long the old data takes to disappear
  const FADE_IN  = 650; // ms: how long the new data takes to appear

  // stage 1: fade all current data marks to zero opacity
  // we select all three chart element types at once so they all disappear together,
  // making it visually clear to the user that the dataset is being swapped out
  const old_marks = d3.selectAll("circle.student-point, path.student-profile, rect.grouped-bar");
  old_marks.transition().duration(FADE_OUT).ease(d3.easeQuadIn)
    .attr("opacity", 0);

  // stage 2: wait for the fade-out to finish before drawing the new data
  // setTimeout delays the redraw by the same duration as the fade-out transition above,
  // so the new dataset only appears on screen after the old one has fully disappeared
  setTimeout(() => {
    // redraw all three charts with the newly filtered dataset
    renderDashboard();

    // stage 3: animate bar heights from 0 up to their final values
    // bars were just drawn at full height by renderDashboard; we immediately
    // override them to start at the baseline (height 0) and animate upward,
    // so the bars appear to grow out of the x-axis rather than popping in all at once
    d3.selectAll("rect.grouped-bar")
      .each(function () {
        // read the final y position and height that renderDashboard just drew
        const rect = d3.select(this);
        const final_y = +rect.attr("y");
        const final_h = +rect.attr("height");
        const base_y  = final_y + final_h; // baseline is the bottom edge of the bar

        // set the bar to zero height at the baseline, then transition up to full height
        rect
          .attr("y", base_y)
          .attr("height", 0)
          .attr("opacity", 0)
          .transition().duration(FADE_IN).ease(d3.easeQuadOut)
          .attr("y", final_y)
          .attr("height", final_h)
          .attr("opacity", rect.attr("data-base-opacity"));
      });

    // stage 4: fade scatter circles and parallel lines in from zero opacity
    // matching the FADE_IN duration to the bar grow above keeps all three charts
    // in sync so everything finishes settling at the same time
    d3.selectAll("circle.student-point, path.student-profile")
      .attr("opacity", 0)
      .transition().duration(FADE_IN).ease(d3.easeQuadOut)
      .attr("opacity", function () { return +d3.select(this).attr("data-base-opacity"); });

  }, FADE_OUT); // delay matches the fade-out duration so stages don't overlap
}
...
// HW3 addition: rectangular d3.brush() for selection interaction.
function renderScatter(container, data, width, height, is_mini) {
  ...
  // d3 joins one circle to each student row,
  // encoding alcohol index (x), grade (y), absences (radius), study time (color)
  ...
  // only add the brush when scatter is the big focus view — the mini card is
  // too small to drag a selection on and its click is already used for swapping charts
  if (!is_mini) {
    // brush() gives us the draggable selection rectangle on the scatter plot
    const brush = d3.brush()
      .extent([[0, 0], [inner_width, inner_height]])
      .on("brush", brushed) // fires continuously while dragging
      .on("end", brushEnd); // fires once when the mouse is released

    // layer the brush on top so it receives mouse events above the circles
    const brush_group = plot.append("g").attr("class", "brush").call(brush);

    // double click anywhere on the brush to clear it out
    brush_group.on("dblclick", () => {
      brush.move(brush_group, null); // passing null removes the visible rectangle
      brushed_ids = null;
      updateBrushBadge();
      applyGlobalHighlight();
    });

    // runs on every mouse move while dragging so the highlight updates as you draw
    function brushed(event) {
      if (!event.selection) return;
      const [[x0, y0], [x1, y1]] = event.selection;

      // filter down to students whose circle centre falls inside the brush rectangle
      brushed_ids = data
        .filter(d => {
          const cx = x(d.alcoholIndex) + jitter(d, 7);
          const cy = y(d.G3);
          return cx >= x0 && cx <= x1 && cy >= y0 && cy <= y1;
        })
        .map(d => d.student_id);

      // console.log(brushed_ids.length, "students in brush");
      updateBrushBadge();
      applyGlobalHighlight();
    }

    // if the mouse was released without actually dragging anything, reset the selection
  ...
  // d3 adds a brush usage hint for the user
  legend.append("text").attr("class", "annotation").attr("y", 258)
    .attr("fill", PALETTE.copper).attr("font-weight", 700).text("drag to brush-select");
  legend.append("text").attr("class", "annotation").attr("y", 272)
    .attr("fill", PALETTE.copper).text("students · dbl-click");
  legend.append("text").attr("class", "annotation").attr("y", 286)
    .attr("fill", PALETTE.copper).text("or Esc to clear");
}

// --- grouped bar chart ---
// summary: average final grades grouped by study time and alcohol level.
// HW3: bars animate in/out on dataset filter changes (handled by renderDashboardAnimated).
function renderGroupedBar(container, data, width, height, is_mini) {
  ...
  // d3 rollup creates a nested map: studytime → alcoholGroup → { avgG3, count }
  ...
  // two-level band scale, x0 positions study time groups, x1 positions alcohol bars inside each group
  ...
  // d3 adds threshold labels explaining how alcohol groups were calculated
...
// HW3 addition: click a line to pin that student for cross-view comparison (Selection interaction).
function renderParallel(container, data, width, height, is_mini) {
  ...
  // scalePoint spaces the vertical axes evenly; padding keeps outermost labels from clipping
  ...
  const paths = plot.selectAll("path.student-profile")
    .data(data)
    .join("path")
    ...
    .on("mouseenter", function (event, d) {
      // don't override the pinned highlight with a hover
      if (pinned_student_id !== null) return;
      // store this student's id and group then sync highlighting across all charts
      hovered_student_id    = d.student_id;
      hovered_alcohol_group = d.alcoholGroup;
      applyGlobalHighlight();
  ...
  // clicking a line locks its highlight across all three views so you can
  // keep comparing it while hovering other things — click it again to unlock
  if (!is_mini) {
    paths.on("click", function (event, d) {
      event.stopPropagation(); // stop this click from bubbling up and triggering the card swap

      if (pinned_student_id === d.student_id) {
        // clicking the same line again clears the pin
        pinned_student_id     = null;
        hovered_student_id    = null;
        hovered_alcohol_group = null;
      } else {
        // lock onto this student across all three charts
        pinned_student_id     = d.student_id;
        hovered_student_id    = d.student_id;
        hovered_alcohol_group = d.alcoholGroup;
        // move this path to the end of the svg so it draws on top of everything
        d3.select(this).raise();
      }
    ...
    // clicking empty space on the chart background clears the pin
    svg.on("click", () => {
      if (pinned_student_id === null) return;
      pinned_student_id     = null;
      hovered_student_id    = null;
  ...
  if (show_guides) {
    ...
    svg.append("text").attr("class", "annotation")
      ...
      .text("compare across");
    svg.append("text").attr("class", "annotation")
      .attr("x", guide_x).attr("y", guide_y + 156).style("font-size", "14px")
      .text("all views.");
  }
  ...
  svg.append("text").attr("class", "annotation")
    ...
    .text("Follow a line across axes to compare drinking, lifestyle, absences, and final grade. Click to pin.");
...
// updates opacity and stroke on every chart element based on whatever is
// currently active, a pin locks in one student, a brush highlights a region,
// hovering a bar lights up a group; pin takes priority over brush, brush over hover
function applyGlobalHighlight() {
  const has_pin     = pinned_student_id    !== null;
  const has_brush   = brushed_ids         !== null;
  ...
  let active_id = null;
  if (has_pin) active_id = pinned_student_id;
  else if (has_student) active_id = hovered_student_id;

  const active_student = active_id
    ? all_rows.find(d => d.student_id === active_id)
    : null;

  // dim or highlight scatter circles depending on what's active
  d3.selectAll("circle.student-point")
    .attr("opacity", function (d) {
      const base = +d3.select(this).attr("data-base-opacity");

      // pin/hover takes priority: dim everything that isn't the active student
      if (active_student) return d.student_id === active_id ? 1 : 0.10;

      // brush: dim students outside the selection rectangle
      if (has_brush) return brushed_ids.includes(d.student_id) ? 0.92 : 0.10;

      // group hover from the bar chart: highlight the matching alcohol group
      if (has_group) return d.alcoholGroup === hovered_alcohol_group ? 0.95 : 0.18;

      return base;
    })
    .attr("stroke", function (d) {
      if (active_student && d.student_id === active_id) return PALETTE.hoverRed;
      return d3.select(this).attr("data-base-stroke");
    })
    .attr("stroke-width", function (d) {
      if (active_student && d.student_id === active_id) return 2.4;
      return +d3.select(this).attr("data-base-stroke-width");
    });

  // same logic for parallel coordinate lines
  d3.selectAll("path.student-profile")
    .attr("opacity", function (d) {
      const base = +d3.select(this).attr("data-base-opacity");
      if (active_student) return d.student_id === active_id ? 1 : 0.035;
      if (has_brush) return brushed_ids.includes(d.student_id) ? 0.55 : 0.04;
      if (has_group) return d.alcoholGroup === hovered_alcohol_group ? 0.45 : 0.05;
      return base;
    })
    .attr("stroke", function (d) {
      if (active_student && d.student_id === active_id) return PALETTE.hoverRed;
    ...
    .attr("stroke-width", function (d) {
      if (active_student && d.student_id === active_id) return 3.5;
      return +d3.select(this).attr("data-base-stroke-width");
    })
    .each(function (d) {
      ...
      if (active_student && d.student_id === active_id) d3.select(this).raise();
    });

  // for the bar chart, highlight the specific study+alcohol bar that matches the active student,
  // or the whole alcohol group column when hovering directly from the bar chart
  d3.selectAll("rect.grouped-bar")
    .attr("opacity", function (d) {
      const base = +d3.select(this).attr("data-base-opacity");
      if (active_student) {
        const match = d.alcoholGroup === active_student.alcoholGroup &&
                      d.study        === String(active_student.studytime);
        return match ? 1 : 0.12;
      }
      // bars show group averages not individual students, so light up any bar
      // that at least one of the brushed students would be counted in
      if (has_brush) {
        // this re-filters brushed_rows once per bar which is a bit repetitive,
        // but the dataset is small enough that it doesn't matter here
        const brushed_rows = all_rows.filter(r => brushed_ids.includes(r.student_id));
        const has_match = brushed_rows.some(r =>
          r.alcoholGroup === d.alcoholGroup && String(r.studytime) === d.study
        );
        return has_match ? 0.92 : 0.18;
      }
      if (has_group) return d.alcoholGroup === hovered_alcohol_group ? 1 : 0.15;
      return base;
    })
    .attr("stroke", function (d) {
      if (!active_student) return "none";
      const match = d.alcoholGroup === active_student.alcoholGroup &&
                    d.study        === String(active_student.studytime);
      return match ? PALETTE.hoverRed : "none";
    })
    .attr("stroke-width", function (d) {
      if (!active_student) return 0;
      const match = d.alcoholGroup === active_student.alcoholGroup &&
                    d.study        === String(active_student.studytime);
      return match ? 2.5 : 0;
    });
...
// shows a count badge when students are selected, hides it when nothing is brushed
function updateBrushBadge() {
  if (brushed_ids === null || brushed_ids.length === 0) {
    brush_badge.classed("is-active", false).text("");
  } else {
    brush_badge
      .classed("is-active", true)
      .text(`${brushed_ids.length} student${brushed_ids.length === 1 ? "" : "s"} selected · Click to Clear`);
  }
}

// shows the pinned student label with their grade, clears it when nothing is pinned
// d is optional — only passed when we have the datum handy to pull the grade from
function updatePinLabel(d) {
  if (!pinned_student_id) {
    pin_label.classed("is-active", false).text("");
  } else {
    const grade = d ? ` · G3 = ${d.G3}` : "";
    pin_label
      .classed("is-active", true)
      .text(`Student pinned ${grade} · Click to Unpin`);
  }
}
