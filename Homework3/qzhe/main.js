let abFilter = 25;
const width = window.innerWidth;
const height = window.innerHeight;

//the place and size of scatter plot
let scatterLeft = 700, scatterTop = 0;
let scatterMargin = {top: 20, right: 40, bottom: 50, left: 70},
scatterWidth = 600 - scatterMargin.left - scatterMargin.right,
scatterHeight = 350 - scatterMargin.top - scatterMargin.bottom;

//the place and size of bar Chart
let teamLeft = 0, teamTop = 0;
let teamMargin = {top: 20, right: 40, bottom: 50, left: 70},
teamWidth = 600 - teamMargin.left - teamMargin.right,
teamHeight = 350 - teamMargin.top - teamMargin.bottom;

////the place and size of Sankey diagram
let distrLeft = 0, distrTop = 350;
let distrMargin = {top: 20, right: 40, bottom: 50, left: 70},
distrWidth = width - 200 - distrMargin.left - distrMargin.right,
distrHeight = height - 350 - distrMargin.top - distrMargin.bottom;




// plots
//read the dataset
d3.csv("data/mxmh_survey_results.csv").then(rawData =>{
    console.log("rawData", rawData);

    //check the dataset, change string into number
    //create age group to classify
    rawData.forEach(function(d){
        d["Hours per day"] = Number(d["Hours per day"]);
        d.Age = Number(d.Age);

        if(d.Age < 18){
            d.ageGroup = "<18";
        }
        else if(d.Age <= 25){
            d.ageGroup = "18-25";
        }
        else if(d.Age <= 35){
            d.ageGroup = "26-35";
        }
        else if(d.Age <= 50){
            d.ageGroup = "36-50";
        }
        else{
            d.ageGroup = "50+";
        }

    });


    //Scatter plot
    const svg = d3.select("svg");

    //different Primary streaming service will use different color
    const colorScale = d3.scaleOrdinal(d3.schemeTableau10).domain(rawData.map(d => d['Primary streaming service']));

    //create group of the scatter plot

    const g1 = svg.append("g")
                .attr("width", scatterWidth + scatterMargin.left + scatterMargin.right)
                .attr("height", scatterHeight + scatterMargin.top + scatterMargin.bottom)
                .attr("transform", `translate(${scatterLeft + scatterMargin.left}, ${scatterTop + scatterMargin.top})`);


    
    //add the title, make it stay in the top of graph
    g1.append("text")
    .attr("x", scatterWidth / 2)
    .attr("y", 5)
    .attr("font-size", "20px")
    .attr("text-anchor", "middle")
    .text("Age vs Daily Listening Hours");

    // X label(age)
    g1.append("text")
    .attr("x", scatterWidth / 2)
    .attr("y", scatterHeight + 50)
    .attr("font-size", "15px")
    .attr("text-anchor", "middle")
    .text("Age");


    // Y label(hours per day), rotate 90
    g1.append("text")
    .attr("x", -(scatterHeight / 2))
    .attr("y", -40)
    .attr("font-size", "15px")
    .attr("text-anchor", "middle")
    .attr("transform", "rotate(-90)")
    .text("Hours Per Day");

    // X ticks, map the age to the horizontal position of the scatter plot
    const x1 = d3.scaleLinear()
    .domain([0, d3.max(rawData, d => d.Age)])
    .range([0, scatterWidth]);

    //create x axis
    const xAxisCall = d3.axisBottom(x1).ticks(7);
    g1.append("g")
    .attr("transform", `translate(0, ${scatterHeight})`)
    .call(xAxisCall)
    .selectAll("text")
        .attr("y", "10")
        .attr("x", "-5")
        .attr("text-anchor", "end")
        .attr("transform", "rotate(-40)");

    // Y ticks(map the hours to the vertical position of the scatter plot)
    const y1 = d3.scaleLinear()
    .domain([0, d3.max(rawData, d => d["Hours per day"])])
    .range([scatterHeight, 0]);

    //create y axis
    const yAxisCall = d3.axisLeft(y1).ticks(13);
    g1.append("g").call(yAxisCall);


    // circles, for every data
    const circles = g1.selectAll("circle").data(rawData);

    const dot = circles.enter().append("circle")
         .attr("cx", d => x1(d.Age))
         .attr("cy", d => y1(d["Hours per day"]))
         .attr("r", 3
         )
         .attr("fill", d => colorScale(d['Primary streaming service']))
         .attr("opacity", 0.4);

    //get all streaming services, creating the legend
    const services_color = colorScale.domain();

    const legend = g1.append("g")
        .attr("transform", `translate(${scatterWidth - 100}, 0)`);

        //every service get different color
    services_color.forEach((service, i) => {
        const legendRow = legend.append("g")
            .attr("transform", `translate(0, ${i * 15})`);

        //color
        legendRow.append("rect")
            .attr("width", 10)
            .attr("height", 10)
            .attr("fill", colorScale(service))
            .attr("opacity", 0.7);

            //text to explain each color
        legendRow.append("text")
            .attr("x", 20)
            .attr("y", 10)
            .attr("text-anchor", "start")
            .style("text-transform", "capitalize")
            .attr("font-size", "12px")
            .text(service);
    });

    //interaction: brushing, Overlay a draggable rectangular selection box on the scatter plot
    //if the point is in the selected area, make it brighter and increase the radius
    //else, reduce the transparency
    g1.append("g")
        .attr("class", "bursh")
        .call(d3.brush()
        //check the area of scatter plot
            .extent([[0,0], [scatterWidth, scatterHeight]])
            .on("start brush end", function(event) {
                const selection = d3.event.selection;

                //if this area is brushed
                if (selection) {
                    const [[x0, y0], [x1B, y1B]] = selection;
                    value = dot
                        .attr("opacity", 0.5)
                        .attr("r", 3)
                        .filter(d => x0 <= x1(d.Age) && x1(d.Age) < x1B && 
                        y0 <= y1(d["Hours per day"]) && y1(d["Hours per day"]) < y1B)
                    .attr("fill", d => colorScale(d["Primary streaming service"]))
                    .attr("opacity", 1)
                    .attr("r", 4);
                    //if the area isn't brushed
                } else {
                    dot
                        .attr("fill", d => colorScale(d["Primary streaming service"]))
                        .attr("opacity", 0.5)
                        .attr("r", 3);
                    }
        }));





    //Bar Chart

    //depend on different service, counting total hours using hours per day
    const hours = rawData.reduce((s, d) => {
        const service = d['Primary streaming service'];
        const hours = Number(d['Hours per day']);
        s[service] = hours +  (s[service] || 0);
        return s;
    }, {});

    //change the data to array
    const bar = Object.keys(hours).map((key) => ({ service: key, total_hours: hours[key] }));
    console.log("bar", bar);

    //create bar chart group
    const g2 = svg.append("g")
                .attr("width", teamWidth + teamMargin.left + teamMargin.right)
                .attr("height", teamHeight + teamMargin.top + teamMargin.bottom)
                .attr("transform", `translate(${teamMargin.left}, ${teamMargin.top})`);
    
    //title for bar chart, move it to the top of the graph
    g2.append("text")
    .attr("x", scatterWidth / 2)
    .attr("y", 5)
    .attr("font-size", "20px")
    .attr("text-anchor", "middle")
    .text("Total Listening Hours for each Streaming Service");


    // X label(Primary Streaming Service)
    g2.append("text")
    .attr("x", teamWidth / 2)
    .attr("y", teamHeight + 50)
    .attr("font-size", "15px")
    .attr("text-anchor", "middle")
    .text("Primary Streaming Service");


    // Y label(Total Hours)
    g2.append("text")
    .attr("x", -(teamHeight / 2))
    .attr("y", -40)
    .attr("font-size", "15px")
    .attr("text-anchor", "middle")
    .attr("transform", "rotate(-90)")
    .text("Total Hours");

    // X ticks(each service corresponds to a specific pillar position.)
    const x2 = d3.scaleBand()
    .domain(bar.map(d => d.service))
    .range([0, teamWidth])
    .paddingInner(0.3)
    .paddingOuter(0.2);

    const xAxisCall2 = d3.axisBottom(x2);
    g2.append("g")
    .attr("transform", `translate(0, ${teamHeight})`)
    .call(xAxisCall2)
    .selectAll("text")
        .attr("y", "10")
        .attr("x", "-5")
        .attr("text-anchor", "end")
        .attr("transform", "rotate(-40)");

    // Y ticks(Each total hour corresponds to a specific pillar position.)
    const y2 = d3.scaleLinear()
    .domain([0, d3.max(bar, d => d.total_hours)])
    .range([teamHeight, 0])
    .nice();

    const yAxisCall2 = d3.axisLeft(y2)
                        .ticks(6);
    g2.append("g").call(yAxisCall2);

    // bars\
    //make the bar chart grow from the bottom, begin at x axis
    //Use transition to make the columns gradually grow from the bottom to their actual height.

    const bars = g2.selectAll("rect").data(bar);

    bars.enter().append("rect")
    
        .attr("x", d => x2(d.service))
        .attr("width", x2.bandwidth())
        .attr("fill", d => colorScale(d.service))

        //at the beginning, start from bottom of x axis
        .attr("y", teamHeight)
        .attr("height", 0)

        .transition()
        .duration(2500)
        .delay((d,i) => i * 50)

        .attr("y", d => y2(d.total_hours))
        .attr("height", d => teamHeight - y2(d.total_hours));

       


    //Sankey diagram

    //read and store the data, use -> to seperate each part
    //count the number for each node(age_service and service_genre)
    //get source(begin node), target(end node)
    //add source, target into node, check if they're already there or not
    //create links by index from node

    const graph_data = rawData.reduce((s, d) => {
        const age_service = d.ageGroup + " -> " + d["Primary streaming service"];
        const service_genre = d["Primary streaming service"] + " -> " + d["Fav genre"];

        s[age_service] = 1 + (s[age_service] || 0);
        s[service_genre] = 1 + (s[service_genre] || 0);

        return s;
    }, {});

    //nodes: age->service -> genre
    const graph = {
        nodes: [],
        links: []
    };

    //check if the node is appeard before
    const node_info = {};

    //push data into ndoe and links
    Object.keys(graph_data).forEach(function(key){
        const [sourceName, targetName] = key.split(" -> ");

        if(node_info[sourceName] == undefined){
            node_info[sourceName] = graph.nodes.length;
            graph.nodes.push({name: sourceName});
        }

        if(node_info[targetName] == undefined){
            node_info[targetName] = graph.nodes.length;
            graph.nodes.push({name: targetName});
        }

        graph.links.push({
            source: node_info[sourceName],
            target: node_info[targetName],
            value: graph_data[key]
        });
    });

    


    //create Sankey graph group, move it to the right place
    const g3 = svg.append("g")
            .attr("width", distrWidth + distrMargin.left + distrMargin.right)
            .attr("height", distrHeight + distrMargin.top + distrMargin.bottom)
            .attr("transform", `translate(${distrLeft + distrMargin.left}, ${distrTop + distrMargin.top})`);
    //use different color for different services
      var sankeyColor = d3.scaleOrdinal(d3.schemeTableau10);


    // add the title for the graph, at the bottom of graph
    g3.append("text")
    .attr("x", distrWidth / 2)
    .attr("y", distrHeight)
    .attr("font-size", "20px")
    .attr("text-anchor", "middle")
    .text("Age Group -> Streaming Service -> Favorite Genre");


    // Y label(number of table)
    g3.append("text")
    .attr("x", -(distrHeight / 2))
    .attr("y", -30)
    .attr("font-size", "15px")
    .attr("text-anchor", "middle")
    .attr("transform", "rotate(-90)")
    .text("Number of People");



  
// Set the sankey diagram properties
    var sankey = d3.sankey()
        .nodeWidth(10)
        .nodePadding(20)
        .size([distrWidth, distrHeight]);
// Constructs a new Sankey generator with the default settings.
    sankey
        .nodes(graph.nodes)
        .links(graph.links)
        .layout(1);

   // add in the links
    var link = g3.append("g")
        .selectAll(".link")
        .data(graph.links)
        .enter()
        .append("path")
            .attr("class", "link")
            .attr("d", sankey.link())
            .style("fill", "none")
            .style("stroke", function(d) {
                const services = colorScale.domain();
                let initial_color = "gray";
            
                if (services.includes(d.source.name)) {
                    initial_color = colorScale(d.source.name);
                } else if (services.includes(d.target.name)) {
                    initial_color = colorScale(d.target.name);
                }
                d.initial = initial_color;
                return initial_color;

            })
            
            

            .style("stroke-opacity", 0.7)
            .style("stroke-width", function(d) { return Math.max(1, d.dy); })
            .sort(function(a, b) { return b.dy - a.dy; });

            //animation, at beginning, all lines are grey, and then change the color as the time gose by.
            //use delay to control each line, make them change color in different time.
            link
                .style("stroke", "gray")
                .transition()
                .delay(function(d, i){ return i * 100; })
                .style("stroke", function(d) { return d.initial})

 // add in the nodes
     var node = g3.append("g")
        .selectAll(".node")
        .data(graph.nodes)
        .enter().append("g")
        .attr("class", "node")
        .attr("transform", function(d) { return "translate(" + d.x + "," + d.y + ")"; });

  // add the rectangles for the nodes
    node.append("rect")
        .attr("height", function(d) { return d.dy; })
        .attr("width", sankey.nodeWidth())
        .style("fill", function(d) { return d.color = sankeyColor(d.name); })
        .style("stroke", function(d) { return d3.rgb(d.color).darker(2); })
        .append("title")
            .text(function(d) { return d.name + "\n" + d.value + " people"; });

    
    node.append("text") 
            .attr("x", "15")
            .attr("y", function(d) { return d.dy / 2; })
            .attr("dy", ".35em")
            .attr("text-anchor", "end")
            .attr("transform", null)
            .text(function(d){ return d.name})
            .attr("text-anchor", "start");

    //when select a node, the line related to the node will turn red. and rest will become more light
    // when the mouse is on the node, the lines related to this node will turn red.
    //then, after the mouse leave, color will change back. 
    node.on("mouseover", function(d) {
        link 
            
            .style("stroke-opacity", 0.7)
            .style("stroke", function(d) { return d.initial })

            .filter(function(l){ return l.source.name == d.name || l.target.name == d.name })

            .style("stroke", "red")
            .style("stroke-opacity", 1)

        d3.select(this)
            .attr("fill", "black")
        })

    node.on("mouseout", function(d) {
        link 
            
            .style("stroke-opacity", 0.7)
            .style("stroke", function(d) { return d.initial })

        })

    
   
    }).catch(function(error){
    console.log(error);



});


